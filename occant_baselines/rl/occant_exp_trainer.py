...
from habitat_extensions.utils import observations_to_image, compute_iou_acc, compute_confusion_matrix, topdown_to_image
...
class OccAntExpTrainer(BaseRLTrainer):
    ...
    def _std_to_map_coord(self, tensor_map, gpscompass):
    ...
    def _batched_std_to_map_coord(self, tensor_map, gpscompass):
        # tensor_map = torch.Tensor(map).permute((2, 0, 1)).unsqueeze(0)
        b, f, h, w = tensor_map.shape
    ...
    def _save_map_samples(self, map, env_layout, idx=0):
    ...
    def _save_manip_map_samples(self, metrics, top_down_map, ground_truth_states, state_estimates, environment_layout,
    ...
    def _compute_diff_maps(self, end_maps, start_maps):
    ...
    def _compute_weighted_diff_maps(self, visitation_maps, diff_maps):
        return diff_maps * visitation_maps

    def _compute_accuracy_old(self, state_estimates, starting_map_states, gt_map_states):
    ...
    def _sum_map(self, state_estimates, occ_map):
    ...
    def _compute_accuracy(self, state_estimates, starting_map_states, gt_map_states):
    ...
    def _compute_accuracy_masked(self, state_estimates, starting_map_states, gt_map_states, ground_truth_states):
        map_mask = (gt_map_states[:, 1] > 0.5).float()
